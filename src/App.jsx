...
// --- 6. STORE PAGE VIEW (VERSI TERBARU & STABIL) ---
const StorePage = ({ 
  ...
  // Filter Banner dengan Safety Check agar tidak hilang saat refresh
  ...
  return (
    <div className="min-h-screen bg-white pt-24 md:pt-32 pb-20">
      <div className="max-w-7xl mx-auto px-4 md:px-12">
        ...
        {/* --- TAB HOME: BANNER & BEST SELLER --- */}
        {storeTab === 'home' && (
          <div className="fade-up space-y-10 md:space-y-16 px-2">
            
            {/* 1. Landscape Banner (Utama) */}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              {portraitBanners.map((pb, i) => (
                <div key={pb.firebaseId || i} className="relative overflow-hidden group border-2 border-black bg-zinc-100 aspect-[9/16]">
                  ...
                  <div className="absolute top-2 md:top-4 left-2 md:left-4 bg-white px-1.5 md:px-2 py-0.5 text-[8px] md:text-[10px] font-bold uppercase tracking-widest border border-black text-black">CLIP</div>
                </div>
              ))}
            </div>
          </div>
