...
const MusicPlayer = () => {
  ...
  return (
    <>
      ...
      <button
        onClick={togglePlay}
        className="fixed bottom-6 left-6 z-[1000] w-12 h-12 rounded-full bg-black text-white flex items-center justify-center shadow-lg border border-white/20 hover:bg-[#ea281e] transition-colors"
        style={{ cursor: 'none' }}
      >
...
const Header = ({ isMenuOpen, setIsMenuOpen, setView, lang, setLang, t }) => (
  <header className="fixed top-0 w-full z-[400] px-4 md:px-12 py-4 md:py-6 flex justify-between items-center mix-blend-difference text-white pointer-events-none">
    <div 
      ...
      className="font-heading font-bold text-2xl md:text-3xl tracking-tighter cursor-pointer pointer-events-auto select-none hover:text-[#ea281e] transition-colors"
      style={{ cursor: 'none' }}
    >
      TOKKO.
    </div>
    <div className="flex items-center gap-3 md:gap-6 pointer-events-auto">
      <div className="flex gap-1 md:gap-3 text-[9px] md:text-[10px] font-bold">
        {['jawa', 'id', 'en'].map(l => (
          <button 
            key={l} 
            onClick={() => setLang(l)} 
            className={`uppercase px-1.5 py-0.5 border border-white/20 hover:bg-white hover:text-black transition-colors ${lang === l ? 'bg-white text-black' : ''}`}
            style={{ cursor: 'none' }}
          >
            {l}
          </button>
      ...
      <button 
        ...
        className="flex items-center gap-2 md:gap-3 font-heading uppercase text-xs md:text-sm font-bold tracking-widest group"
        style={{ cursor: 'none' }}
      >
  ...
  return (
    <div className="fixed inset-0 z-[600] bg-black/95 flex items-center justify-center p-4 md:p-6 backdrop-blur-md" onClick={onClose}>
      <div className="bg-white max-w-2xl w-full p-6 md:p-8 border-2 border-black shadow-[10px_10px_0_#ea281e] relative max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <button 
          onClick={onClose} 
          className="absolute top-4 right-4 hover:rotate-90 transition"
          style={{ cursor: 'none' }}
        >
          <X size={28}/>
        </button>
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (targetDate?.seconds) {
        target = new Date(targetDate.seconds * 1000);
    ...
    <section className="h-screen flex flex-col justify-center px-6 md:px-12 pt-10 md:pt-20 relative overflow-hidden">
      <div className="flex items-center gap-4 fade-up" style={{animationDelay: '0.2s'}}>
        ...
        <button 
          ...
          className="hidden md:flex w-40 h-40 rounded-full border border-black items-center justify-center font-heading font-bold text-lg hover:bg-black hover:text-white transition-all"
          style={{ cursor: 'none' }}
        >
          {t?.explore || "EXPLORE"}
        </button>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mt-8 md:mt-12 gap-6 md:gap-8 fade-up" style={{animationDelay: '0.4s'}}>
        ...
        <button 
          onClick={() => setView('store')} 
          className="md:hidden border-2 border-black px-8 py-3 font-heading font-bold text-lg hover:bg-black hover:text-white transition-all uppercase"
          style={{ cursor: 'none' }}
        >
          {t?.explore || "EXPLORE"} STORE
        </button>
      </div>
    </section>
            ...
            >
              <div className="aspect-[4/5] bg-gray-200 mb-6 md:mb-8 overflow-hidden border border-black relative">
                {item.image ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-7xl md:text-8xl font-heading text-white bg-zinc-900 italic">
                    {idx + 1}
                  </div>
                )}
              </div>
...
const StorePage = ({ 
  ...
  const journalEntries = useMemo(() => {
    const infos = (informations || []).map(i => ({ 
      ...i, 
      type: 'NEWS',
      content: i.content || i.desc || '',
      date: i.date || new Date().toLocaleDateString('id-ID')
    }));
    const acts = (activities || []).map(a => ({ 
      ...a, 
      type: 'ACTIVITY', 
      title: a.caption || a.title || '', 
      content: a.desc || a.content || '',
      date: a.date || new Date().toLocaleDateString('id-ID')
    }));
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-12">
          ...
          <div className="flex gap-4 md:gap-8 overflow-x-auto pb-2 w-full md:w-auto no-scrollbar">
            {['home', 'catalog', 'journal', 'status', 'reviews'].map(tab => (
              <button 
                ...
                className={`tab-btn ${storeTab === tab ? 'active' : ''}`}
                style={{ cursor: 'none' }}
              >
        ...
        <div className="relative mb-10 md:mb-16 group px-2">
          ...
          <input 
            ...
            className="w-full border-b border-gray-300 py-4 md:py-6 pl-10 md:pl-12 outline-none font-heading text-lg md:text-2xl uppercase focus:border-black transition-colors bg-transparent" 
            value={searchQuery || ""} 
        ...
        {storeTab === 'home' && (
          <div className="fade-up space-y-10 md:space-y-16 px-2">
            ...
            {landscapeBanners.length > 0 && (
              <div className="w-full mb-8">
                {landscapeBanners.map((lb, i) => (
                  <div 
                    key={lb.firebaseId || i} 
                    className="relative w-full aspect-video md:aspect-[21/9] border-2 border-black overflow-hidden bg-zinc-100 shadow-[8px_8px_0_black] mb-6"
                    onClick={() => handleBannerClick(lb)}
                    style={{ cursor: lb.link ? 'pointer' : 'default' }}
                  >
                    ...
                    {lb.type === 'video' ? (
                      <video 
                        ...
                        onError={(e) => {
                          ...
                          e.target.parentElement.innerHTML = '<div class="w-full h-full flex items-center justify-center bg-zinc-800 text-white font-heading">Video Error</div>';
                        }}
                      />
                    ) : (
                      <img 
            ...
            {portraitBanners.length > 0 && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
                {portraitBanners.map((pb) => (
                  <div 
                    key={pb.firebaseId} 
                    className="relative overflow-hidden group border-2 border-black bg-zinc-100 aspect-[9/16]"
                    onClick={() => handleBannerClick(pb)}
                    style={{ cursor: pb.link ? 'pointer' : 'default' }}
                  >
                    {pb.type === 'video' ? (
                      <video 
                        src={pb.image} 
                      ...
                      <img 
                        ...
                        onError={(e) => {
                          e.target.onerror = null;
                          e.target.src = "https://via.placeholder.com/400x600?text=Portrait+Banner";
                        }}
                      />
                    )}
                            ...
                            return (
                              <div key={index} className="relative">
                                <button
                                  ...
                                  className={`w-full text-left p-4 border-2 transition-all ${userHasVoted ? 'cursor-default' : 'hover:border-black hover:bg-black hover:text-white'} ${userHasVoted ? 'border-gray-300' : 'border-gray-300'}`}
                                  style={{ cursor: userHasVoted ? 'default' : 'none' }}
                                >
                                  ...
                                  {userHasVoted && (
                                    <div className="absolute bottom-0 left-0 h-1 bg-[#ea281e]" style={{ width: `${percentage}%` }}></div>
                                  )}
                                </button>
                                {userHasVoted && (
                                  <div className="absolute top-0 left-0 h-full bg-[#ea281e]/10" style={{ width: `${percentage}%` }}></div>
                                )}
                              </div>
                            );
                          })}
                        ...
                        <div className="text-sm text-gray-500 flex justify-between items-center">
                          ...
                          <span className="text-xs">
                            {poll.createdAt?.seconds ? 
                              new Date(poll.createdAt.seconds * 1000).toLocaleDateString('id-ID') : 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 md:gap-8 pt-10">
              {(catalog || []).filter(i => i.isBestSeller).slice(0, 3).map(item => (
                <div 
                  key={item.firebaseId} 
                  className="store-card p-0 group hover:shadow-xl hover:-translate-y-2 transition-all"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 md:gap-12 fade-up px-2">
            {filteredCatalog.length > 0 ? (
                ...
                >
                  <div className="aspect-square bg-zinc-100 mb-4 md:mb-6 relative border border-black flex items-center justify-center overflow-hidden">
                    {item.image ? (
                      ...
                      <ShoppingBag size={80} className="text-zinc-200"/>
                    )}
                    <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                      <span className="text-white font-heading text-2xl md:text-3xl">{t?.detail || "DETAIL"}</span>
                    </div>
                    <div className="absolute bottom-4 left-4 bg-white px-2 py-1 text-[10px] font-bold border border-black uppercase tracking-widest">
                      {item.app}
                    </div>
                  </div>
              ...
              <div className="col-span-full text-center py-20">
                <p className="text-2xl font-heading text-gray-400">No products found</p>
              </div>
            )}
          </div>
        )}
        ...
        {storeTab === 'journal' && (
          <div className="space-y-12 md:space-y-24 fade-up px-2">
            {journalEntries.length > 0 ? journalEntries.map(entry => (
              <div 
                key={entry.firebaseId} 
                className="flex flex-col md:flex-row gap-8 md:gap-16 group hover:-translate-x-2 transition-transform"
              ...
              >
                <div className="md:w-1/3 aspect-[4/3] border border-black overflow-hidden bg-zinc-100 shrink-0">
                  <img 
                    ...
                    onError={(e) => {
                      e.target.onerror = null;
                      e.target.src = "https://via.placeholder.com/600x400?text=Journal+Image";
                    }}
                  />
                </div>
                <div className="md:w-2/3 flex flex-col justify-center py-4">
                  ...
                  <h2 className="text-4xl md:text-6xl font-heading mb-6 group-hover:text-[#ea281e] transition-colors uppercase leading-none tracking-tighter">
                    {entry.title}
                  </h2>
                  <p className="text-lg text-gray-600 line-clamp-3 leading-relaxed font-mono text-xs uppercase">
                    {entry.content}
                  </p>
                </div>
              </div>
          ...
          <div className="fade-up border-2 border-black mx-2">
            <div className="bg-black text-white p-6 md:p-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="font-mono text-[10px] md:text-xs uppercase tracking-[0.2em] font-bold">
                ...
                <tbody className="divide-y divide-zinc-200 uppercase font-bold">
                  {(transactions || []).length > 0 ? transactions.map(t => (
                    <tr key={t.firebaseId} className="hover:bg-zinc-50 transition-colors">
                      ...
                      <td className="p-4 md:p-8 font-heading text-lg">
                    ...
                    <tr>
                      <td colSpan="4" className="p-20 text-center text-gray-300 font-heading text-2xl uppercase">
                        NO ACTIVE DATA
                      </td>
                    </tr>
                  )}
            ...
            {!reviewType ? (
              <div className="border-2 border-black p-8 md:p-12 text-center bg-white shadow-[15px_15px_0_#ea281e]">
                ...
                <input 
                  ...
                  className="w-full border-b-2 border-black py-4 text-center font-mono outline-none mb-10 text-sm uppercase" 
                  value={commentEmail || ""} 
                ...
                <div className="flex flex-col gap-4">
                  <button 
                    ...
                    className="w-full bg-black text-white py-5 font-bold uppercase tracking-widest hover:bg-[#ea281e] flex justify-center items-center gap-2 transition-colors"
                    style={{ cursor: 'none' }}
                  ...
                  <button 
                    onClick={() => setReviewType('guest')} 
                    className="text-xs font-bold uppercase underline hover:text-[#ea281e] transition-colors"
                    style={{ cursor: 'none' }}
                  >
                    {t?.guestBtn || "WRITE AS GUEST"}
                  </button>
                </div>
              </div>
            ) : (
              <div className="border-2 border-black p-6 md:p-10 space-y-8">
                <div className="flex justify-between items-center">
                  <div className="flex flex-col">
                    ...
                    <span className={`text-[10px] font-bold uppercase ${reviewType === 'customer' ? 'text-green-600' : 'text-orange-500'}`}>
                      {reviewType === 'customer' ? (t?.customer || "CUSTOMER") : (t?.nonCustomer || "GUEST")}
                    </span>
                  </div>
                  <div className="flex gap-1">
                    {[1,2,3,4,5].map(s => (
                      <Star 
                        ...
                        fill={s <= (commentRating || 5) ? "black" : "none"} 
                ...
                <textarea 
                  placeholder={t?.postReview || "Write a review..."} 
                  className="w-full h-40 border-2 border-zinc-100 p-4 outline-none focus:border-black uppercase font-mono text-xs" 
                  value={commentText || ""} 
                  onChange={e => setCommentText(e.target.value)}
                />
                <button 
                  onClick={handlePostComment} 
                  className="w-full bg-black text-white py-5 font-bold uppercase tracking-widest hover:bg-[#ea281e] transition-colors"
                  style={{ cursor: 'none' }}
                ...
                {reviewType === 'guest' && (
                  <p className="text-[10px] text-gray-400 italic text-center uppercase tracking-widest font-bold">
            ...
            <div className="mt-20 space-y-12 md:space-y-16">
              {approvedComments.length > 0 ? approvedComments.map((c, idx) => (
                <div key={c.firebaseId || idx} className="border-l-2 border-black pl-8 relative pb-4">
                  <div className={`absolute top-0 right-0 px-2 py-0.5 text-[8px] font-bold uppercase text-white ${c.role === 'customer' ? 'bg-black' : 'bg-orange-500'}`}>
                    {c.role === 'customer' ? (t?.customer || "CUSTOMER") : (t?.nonCustomer || "GUEST")}
                  </div>
                  <div className="flex justify-between items-start mb-4">
                    <span className="font-heading text-2xl uppercase tracking-tighter leading-none">
                  ...
                  <span className="block mt-6 font-mono text-[10px] text-gray-400 uppercase font-bold">
                    {c.date || new Date().toLocaleDateString('id-ID')}
                  </span>
                </div>
              )) : (
          ...
          <div className="space-y-8">
            {selectedProduct.image && (
              <img 
                ...
                className="w-full h-64 object-cover border border-black shadow-[10px_10px_0_black]" 
                alt="Product"
                onError={(e) => {
                  e.target.onerror = null;
                  e.target.src = "https://via.placeholder.com/800x400?text=Product+Image";
                }}
              />
            )}
            <div className="flex flex-col md:flex-row justify-between gap-6">
              <div>
                <span className="text-[#ea281e] font-mono text-xs font-bold uppercase tracking-widest">
                  {selectedProduct.app}
                </span>
                <h2 className="text-5xl font-heading tracking-tighter italic">{selectedProduct.name}</h2>
              </div>
              <div className="text-4xl font-bold italic uppercase tracking-tighter">{selectedProduct.price}</div>
            </div>
            <p className="text-lg text-gray-600 leading-relaxed border-t border-black pt-8 uppercase font-mono text-xs">
              {selectedProduct.desc || "No description available"}
            </p>
            <a 
              href={`https://wa.me/6281319865384?text=Halo TOKKO, saya berminat order ${encodeURIComponent(selectedProduct.name)}`} 
              target="_blank" 
              rel="noreferrer" 
              className="w-full bg-black text-white py-6 text-center font-heading text-2xl flex items-center justify-center gap-4 hover:bg-[#ea281e] transition-colors"
              style={{ cursor: 'none' }}
            >
        ...
        {selectedJournal && (
          <div className="space-y-8">
            <img 
              ...
              className="w-full aspect-video object-cover border border-black shadow-[10px_10px_0_black]" 
              alt="Journal"
              onError={(e) => {
                e.target.onerror = null;
                e.target.src = "https://via.placeholder.com/800x400?text=Journal+Image";
              }}
            />
            ...
            <div className="text-lg text-gray-600 leading-relaxed whitespace-pre-wrap uppercase font-mono text-xs">
  ...
  <div className="min-h-screen admin-login-bg text-white flex flex-col p-6 md:p-20 relative overflow-hidden">
    ...
    <div className="flex-1 flex flex-col justify-center">
      <h1 className="text-[14vw] md:text-[12vw] font-heading leading-none font-bold mb-10 tracking-tighter fade-up uppercase">
        Admin<br/><span className="text-[#ea281e]">{t?.future || "Tokko."}</span>
      </h1>
      <div className="max-w-md w-full fade-up" style={{animationDelay: '0.2s'}}>
        <form onSubmit={handleAdminLogin} className="space-y-8 md:space-y-12">
          <div className="group relative">
            ...
            <input 
              type="text" 
              className="w-full bg-transparent border-b border-gray-800 py-3 font-heading text-2xl outline-none focus:border-[#ea281e] uppercase transition-all" 
          ...
          <div className="flex items-center gap-6">
            <button 
              type="submit" 
              className="bg-[#ea281e] text-white px-8 py-4 font-heading text-lg uppercase tracking-widest hover:bg-white hover:text-black transition-all font-bold"
              style={{ cursor: 'none' }}
            >
              Masuk
            </button>
            <button 
              type="button" 
              onClick={() => setView('landing')} 
              className="text-gray-500 font-mono text-xs underline uppercase font-bold hover:text-white transition-colors"
              style={{ cursor: 'none' }}
            >
              {t?.logout || "Keluar"}
            </button>
...
const AdminDashboard = ({ 
  ...
  const [trxForm, setTrxForm] = useState({ name: '', app: '', durationDays: '', email: '', status: 'Aktif' });
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col md:flex-row pt-16 md:pt-24">
        ...
        <div className="p-6 space-y-2 h-full overflow-y-auto">
          {tabs.map(tab => (
            <button 
              ...
              className={`w-full text-left font-heading text-lg p-4 border border-black transition-all mb-2 ${activeTab === tab ? 'bg-black text-white' : 'text-gray-400 hover:bg-zinc-50'}`}
              style={{ cursor: 'none' }}
            >
              {tab}
            </button>
          ...
          <div className="mt-6 pt-6 border-t border-black">
            <button 
              ...
              className="w-full flex items-center justify-center gap-2 text-red-600 font-bold text-xs uppercase tracking-widest p-4 border border-red-600 hover:bg-red-600 hover:text-white transition-colors"
              style={{ cursor: 'none' }}
            >
      ...
      <aside className="hidden md:block w-64 border-r-2 border-black p-8 sticky top-24 h-[calc(100vh-6rem)] shrink-0">
        ...
        <nav className="space-y-1">
          {tabs.map(tab => (
            <button 
              ...
              className={`w-full text-left font-heading text-lg p-3 border border-black transition-all mb-2 ${activeTab === tab ? 'bg-black text-white' : 'text-gray-400 hover:bg-zinc-50'}`}
              style={{ cursor: 'none' }}
            >
              {tab}
            </button>
        ...
        <button 
          onClick={handleLogout} 
          className="mt-20 flex items-center gap-2 text-red-600 font-bold text-[10px] uppercase tracking-widest hover:text-red-800 transition-colors"
          style={{ cursor: 'none' }}
        >
          <LogOut size={14}/> LOGOUT SYSTEM
        </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {['Aktif', 'Perbaikan', 'Mati'].map(s => (
                <button 
                  ...
                  className={`p-6 md:p-12 border-2 font-heading text-2xl md:text-4xl transition-all ${globalStatus === s ? 'bg-black text-white border-black' : 'text-zinc-200 border-zinc-100 hover:border-black'}`}
                  style={{ cursor: 'none' }}
                >
                  {s.toUpperCase()}
                </button>
          ...
          <div className="fade-up">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-10 border-b-2 border-black pb-4 gap-4">
              ...
              <button 
                onClick={() => { 
                  setEditingId(null); 
                  setCatForm({ name: '', app: '', price: '', desc: '', isBestSeller: false, imageUrl: '' }); 
                  setIsModalOpen(true); 
                }} 
                className="bg-black text-white px-4 md:px-6 py-2 font-bold uppercase text-xs hover:bg-[#ea281e] transition-colors"
                style={{ cursor: 'none' }}
              >
                ADD NEW
              </button>
                ...
                <div key={item.firebaseId} className="border-2 border-black p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 group bg-white shadow-[4px_4px_0_black]">
                  ...
                  <div className="flex gap-2 w-full md:w-auto justify-end">
                    <button 
                      ...
                      className="p-2 border border-black hover:bg-black transition-colors"
                      style={{ cursor: 'none' }}
                    >
                      <Pencil size={16}/>
                    </button>
                    <button 
                      onClick={() => handleDelete('catalog', item.firebaseId)} 
                      className="p-2 border border-[#ea281e] text-[#ea281e] hover:bg-[#ea281e] hover:text-white transition-colors"
                      style={{ cursor: 'none' }}
                    >
                      <Trash2 size={16}/>
                    </button>
          ...
          <div className="fade-up">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-10 border-b-2 border-black pb-4 gap-4">
              ...
              <button 
                onClick={() => { 
                  setEditingId(null); 
                  setInfoForm({ title: '', content: '', imageUrl: '', isFeatured: false }); 
                  setIsModalOpen(true); 
                }} 
                className="bg-black text-white px-4 md:px-6 py-2 font-bold uppercase text-xs hover:bg-[#ea281e] transition-colors"
                style={{ cursor: 'none' }}
                ...
                <div key={info.firebaseId} className="border-2 border-black p-4 flex flex-col md:flex-row gap-4 md:gap-6 items-start md:items-center group bg-white shadow-[4px_4px_0_black]">
                  <div className="w-full md:w-20 h-20 border border-zinc-200 overflow-hidden shrink-0">
                    <img 
                      ...
                      onError={(e) => {
                        e.target.onerror = null;
                        e.target.src = "https://via.placeholder.com/100x100?text=News+Image";
                      }}
                    />
                  </div>
                  ...
                  <div className="flex gap-2 w-full md:w-auto justify-end">
                    ...
                    <button 
                      onClick={() => handleDelete('informations', info.firebaseId)} 
                      className="p-2 border border-[#ea281e] text-[#ea281e] hover:bg-[#ea281e] hover:text-white transition-colors"
                      style={{ cursor: 'none' }}
                    >
                      <Trash2 size={16}/>
          ...
          <div className="fade-up">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-10 border-b-2 border-black pb-4 gap-4">
              ...
              <button 
                onClick={() => { 
                  setEditingId(null); 
                  setBanForm({ title: '', desc: '', imageUrl: '', type: 'image', orientation: 'landscape', link: '' }); 
                  setIsModalOpen(true); 
                }} 
                className="bg-black text-white px-4 md:px-6 py-2 font-bold uppercase text-xs hover:bg-[#ea281e] transition-colors"
                style={{ cursor: 'none' }}
                ...
                <div key={b.firebaseId} className="border-2 border-black p-4 bg-white shadow-[4px_4px_0_black] md:shadow-[6px_6px_0_black] group">
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-2 md:gap-0">
                    ...
                    <div className="flex gap-2">
                      <button 
                        onClick={() => openEdit('banner', b)} 
                        className="p-1.5 border border-black hover:bg-black transition-colors"
                        style={{ cursor: 'none' }}
                      >
                        <Pencil size={14}/>
                      </button>
                      <button 
                        onClick={() => handleDelete('banners', b.firebaseId)} 
                        className="p-1.5 border border-[#ea281e] text-[#ea281e] hover:bg-[#ea281e] hover:text-white transition-colors"
                        style={{ cursor: 'none' }}
                      >
                        <Trash2 size={14}/>
                      </button>
          ...
          <div className="fade-up">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-10 border-b-2 border-black pb-4 gap-4">
              ...
              <button 
                onClick={() => { 
                  setEditingId(null); 
                  setPollForm({ question: '', options: ['', ''], active: true }); 
                  setIsModalOpen(true); 
                }} 
                className="bg-black text-white px-4 md:px-6 py-2 font-bold uppercase text-xs hover:bg-[#ea281e] transition-colors"
                style={{ cursor: 'none' }}
                  ...
                  <div key={poll.firebaseId} className="border-2 border-black p-6 bg-white shadow-[4px_4px_0_black]">
                    ...
                    <div className="flex justify-between items-center mt-4">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button 
                          onClick={() => handleDelete('polls', poll.firebaseId)} 
                          className="p-1.5 border border-[#ea281e] text-[#ea281e] hover:bg-[#ea281e] hover:text-white transition-colors"
                          style={{ cursor: 'none' }}
                        >
                          <Trash2 size={14}/>
          ...
          <div className="fade-up">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 md:mb-10 border-b-2 border-black pb-4 gap-4">
              ...
              <button 
                onClick={() => { 
                  setEditingId(null); 
                  setTrxForm({ name: '', app: '', durationDays: '', email: '', status: 'Aktif' }); 
                  setIsModalOpen(true); 
                }} 
                className="bg-black text-white px-4 md:px-6 py-2 font-bold uppercase text-xs hover:bg-[#ea281e] transition-colors"
                style={{ cursor: 'none' }}
                ...
                <tbody className="divide-y divide-zinc-200">
                  {(transactions || []).map(t => (
                    <tr key={t.firebaseId} className="hover:bg-zinc-50 transition-colors">
                      ...
                      <td className="p-4 font-heading text-lg">
                        <TimeDisplay targetDate={t.targetDate} status={t.status}/>
                      </td>
                      <td className="p-4 text-right">
                        <div className="flex justify-end gap-3">
                          <button 
                            onClick={() => openEdit('trx', t)} 
                          ...
                          <button 
                            onClick={() => handleDelete('transactions', t.firebaseId)} 
                            className="text-[#ea281e] hover:scale-110 transition-transform"
                            style={{ cursor: 'none' }}
                          >
                            <Trash2 size={16}/>
                          </button>
                ...
                <div key={c.firebaseId} className="border-2 border-orange-500 bg-orange-50 p-4 md:p-6 lg:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-6 shadow-[4px_4px_0_#f97316] md:shadow-[6px_6px_0_#f97316]">
                  ...
                  <div className="flex gap-3 w-full md:w-auto">
                    <button 
                      ...
                      className="flex-1 bg-green-600 text-white px-4 md:px-6 py-2 md:py-3 font-bold uppercase text-[10px] border border-black shadow-[2px_2px_0_black] md:shadow-[4px_4px_0_black] hover:bg-green-700 transition-colors"
                      style={{ cursor: 'none' }}
                    >
                      APPROVE
                    </button>
                    <button 
                      ...
                      className="flex-1 bg-red-600 text-white px-4 md:px-6 py-2 md:py-3 font-bold uppercase text-[10px] border border-black shadow-[2px_2px_0_black] md:shadow-[4px_4px_0_black] hover:bg-red-700 transition-colors"
                      style={{ cursor: 'none' }}
                    >
                      REJECT
                    </button>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={`MGMT: ${activeTab}`}>
        {activeTab === 'CATALOG' && (
          <div className="space-y-6">
            <input 
              ...
              className="w-full border-b-2 border-black py-3 outline-none font-heading text-xl md:text-2xl uppercase" 
              value={catForm.name} 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input 
                ...
                className="w-full border-b border-zinc-100 py-3 outline-none uppercase font-bold text-[10px]" 
                value={catForm.app} 
            ...
            <input 
              placeholder="IMAGE URL (data:image/png;base64,... or https://)" 
              className="w-full border-b-2 border-black py-3 outline-none font-mono text-xs" 
              value={catForm.imageUrl} 
              onChange={e => setCatForm({...catForm, imageUrl: e.target.value})}
            />
            <textarea 
              placeholder="DESC" 
              className="w-full h-32 border border-zinc-100 p-3 font-mono text-xs uppercase" 
              value={catForm.desc} 
            ...
            <button 
              onClick={async () => { 
                await handleSaveCatalog(catForm, editingId); 
                setIsModalOpen(false); 
                setCatForm({ name: '', app: '', price: '', desc: '', isBestSeller: false, imageUrl: '' });
                setEditingId(null);
              }} 
              className="w-full bg-black text-white py-4 md:py-5 font-bold uppercase tracking-widest hover:bg-[#ea281e] transition-colors"
              style={{ cursor: 'none' }}
            >
              COMMIT CHANGES
            </button>
        ...
        {activeTab === 'NEWS' && (
          <div className="space-y-6">
            ...
            <input 
              ...
              onChange={e => setInfoForm({...infoForm, imageUrl: e.target.value})}
            />
            <textarea 
              placeholder="CONTENT" 
              className="w-full h-48 border border-zinc-100 p-3 font-mono text-xs uppercase" 
              value={infoForm.content} 
              onChange={e => setInfoForm({...infoForm, content: e.target.value})}
            />
            <button 
              onClick={async () => { 
                await handleSaveInfo(infoForm, editingId); 
                setIsModalOpen(false); 
                setInfoForm({ title: '', content: '', imageUrl: '', isFeatured: false });
                setEditingId(null);
              }} 
        ...
        {activeTab === 'BANNERS' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <select 
                className="p-3 border border-black font-bold uppercase text-[10px]" 
                value={banForm.type} 
              ...
              <select 
                className="p-3 border border-black font-bold uppercase text-[10px]" 
                value={banForm.orientation} 
                onChange={e => setBanForm({...banForm, orientation: e.target.value})}
              >
            ...
            <input 
              placeholder="IMAGE/VIDEO URL (data:image/png;base64,... or https://)" 
              className="w-full border-b-2 border-black py-3 outline-none font-mono text-xs" 
              value={banForm.imageUrl} 
              onChange={e => setBanForm({...banForm, imageUrl: e.target.value})}
            />
            <input 
              placeholder="LINK (optional) e.g. https://example.com" 
              className="w-full border-b-2 border-black py-3 outline-none font-mono text-xs" 
              value={banForm.link} 
              onChange={e => setBanForm({...banForm, link: e.target.value})}
            />
            <button 
              onClick={async () => { 
                const s = await handleSaveBanner(banForm, editingId); 
                if (s) {
                  setIsModalOpen(false); 
                  setBanForm({ title: '', desc: '', imageUrl: '', type: 'image', orientation: 'landscape', link: '' });
                  setEditingId(null);
          ...
          <div className="space-y-6">
            ...
            <label className="flex items-center gap-2 font-bold uppercase text-[10px] tracking-widest cursor-pointer">
              <input 
                ...
                onChange={e => setPollForm({...pollForm, active: e.target.checked})}
              /> 
              ACTIVE POLL
            </label>
            ...
            <button 
              onClick={async () => { 
                await handleSavePoll(pollForm, editingId); 
                setIsModalOpen(false); 
                setPollForm({ question: '', options: ['', ''], active: true });
                setEditingId(null);
              }} 
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input 
                ...
                className="w-full border-b border-zinc-100 py-3 outline-none font-bold text-[10px] uppercase" 
                value={trxForm.app} 
                onChange={e => setTrxForm({...trxForm, app: e.target.value})}
              />
              <input 
                type="number" 
                placeholder="DAYS (DURASI)" 
                className="w-full border-b border-zinc-100 py-3 outline-none font-bold text-[10px] uppercase" 
            ...
            <input 
              ...
              className="w-full border-b border-zinc-100 py-3 outline-none font-mono text-[10px] font-bold" 
              value={trxForm.email} 
              onChange={e => setTrxForm({...trxForm, email: e.target.value})}
            />
            <button 
              onClick={async () => { 
                await handleSaveTransaction(trxForm, editingId); 
                setIsModalOpen(false); 
                setTrxForm({ name: '', app: '', durationDays: '', email: '', status: 'Aktif' });
                setEditingId(null);
              }} 
...
export default function App() {
  ...
  const subscribeData = () => {
    // Global Status
    const unsubscribeGlobal = onSnapshot(doc(db, "settings", "global_status"), (d) => {
      if (d.exists()) {
        setGlobalStatus(d.data().status || 'Aktif');
      } else {
        // Create default if not exists
        setDoc(doc(db, "settings", "global_status"), { status: 'Aktif' });
      }
    });

    // Catalog
    const unsubscribeCatalog = onSnapshot(
      query(collection(db, "catalog"), orderBy("id", "desc")), 
      (s) => setCatalog(s.docs.map(d => ({...d.data(), firebaseId: d.id})))
    );

    // Informations
    const unsubscribeInfo = onSnapshot(
      query(collection(db, "informations"), orderBy("id", "desc")), 
      (s) => setInformations(s.docs.map(d => ({...d.data(), firebaseId: d.id})))
    );

    // Transactions
    const unsubscribeTrans = onSnapshot(
      query(collection(db, "transactions"), orderBy("id", "desc")), 
      (s) => setTransactions(s.docs.map(d => ({...d.data(), firebaseId: d.id})))
    );

    // Banners
    const unsubscribeBanners = onSnapshot(
      query(collection(db, "banners"), orderBy("id", "desc")), 
      (s) => setBanners(s.docs.map(d => ({...d.data(), firebaseId: d.id})))
    );

    // Polls
    const unsubscribePolls = onSnapshot(
      query(collection(db, "polls"), orderBy("createdAt", "desc")), 
      (s) => setPolls(s.docs.map(d => ({...d.data(), firebaseId: d.id})))
    );

    // Activities
    const unsubscribeActivities = onSnapshot(
      query(collection(db, "activities"), orderBy("id", "desc")), 
      (s) => setActivities(s.docs.map(d => ({...d.data(), firebaseId: d.id})))
    );

    // Comments
    const unsubscribeComments = onSnapshot(
      query(collection(db, "comments"), orderBy("timestamp", "desc")), 
      (s) => setComments(s.docs.map(d => ({...d.data(), firebaseId: d.id})))
    );
  ...
  const handleVerifyEmail = async () => {
    ...
    try {
      ...
      if (!snap.empty) {
        ...
        alert("Email tidak terdaftar sebagai pelanggan. Anda dapat menulis sebagai tamu.");
      }
    } catch(e) { 
  ...
  const handleVote = async (pollId, optionIndex) => {
    try {
      ...
      const pollDoc = await getDocs(query(collection(db, "polls"), where("__name__", "==", pollId)));
      ...
      const pollData = pollDoc.docs[0].data();
  ...
  const handleSaveInfo = async (data, editId) => {
    try {
      ...
      if (editId) {
        await updateDoc(doc(db, "informations", editId), payload);
        alert("Journal updated successfully!");
      } else {
        await addDoc(collection(db, "informations"), payload);
